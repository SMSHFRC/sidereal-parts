...
如果管理者已經在 Onshape Developer Portal 設定 sidereal-parts Extension，隊員可以直接從 Onshape 文件右側面板匯入。

使用方式:

1. 打開 Onshape assembly。
2. 在右側面板開啟 sidereal-parts app。
3. 若尚未登入系統，先在面板內登入。
4. 選擇系統、加工方式、材料與後處理。
5. 按「預覽」查看 BOM 分流。
6. 按「匯入」建立/更新任務。

右側面板使用的仍是同一套 M3 匯入流程，所以資料會同步出現在正式網站看板。
...
- Cloudflare Pages iframe header 設定
...
M4: Onshape 右側面板入口，可在 CAD 內預覽並匯入 BOM
```
